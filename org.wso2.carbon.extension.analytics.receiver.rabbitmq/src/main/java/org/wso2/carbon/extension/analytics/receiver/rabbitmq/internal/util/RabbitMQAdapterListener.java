...
public class RabbitMQAdapterListener implements Runnable {
    ...
    public void stopListener(String adapterName) {
        if (channel != null && channel.isOpen()) {
            try {
                channel.close();
                log.info("RabbitMQ channel closed for receiver " + adapterName);
            } catch (IOException e) {
    ...
    private Connection makeConnection() throws IOException {
        ...
        try {
            ...
            int retryC = 0;
            while ((connection == null) && ((retryCountMax == -1) || (retryC < retryCountMax))) {
                retryC++;
    ...
    public void setConnected(boolean connectionSucceeded) {
    ...
    private void waitForConnection() throws IOException {
        ...
        while (!connection.isOpen()
            ...
            retryCount++;
            log.info("Attempting to reconnect to RabbitMQ Broker for the receiver " + adapterName + " " +
                    "in" + retryInterval + " ms");
            try {
                Thread.sleep(retryInterval);
            } catch (InterruptedException e) {
